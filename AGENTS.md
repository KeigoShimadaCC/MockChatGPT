...
- Codex threads are stateful and can't be forked, so there is no true history rewrite. "Regenerate"/"edit" (`replaceLast` on the messages endpoint) send an extra instruction turn to the thread and rewrite only the *stored* transcript. Branching (`/api/conversations/:id/branch`) works around the same limit differently: the child copies the transcript, starts with `threadId: null`, and replays that transcript into its first turn as a `<branch_context>` block (`seedPending`, cleared only once a thread actually came back — a failed first turn must still seed on the retry).

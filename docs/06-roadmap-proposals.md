...
"When a CSV lands in ~/Downloads → clean it and append to my report"; "when this webhook fires → run this prompt". Users hit the ceiling of ChatGPT Tasks / Claude Routines immediately (both are time-based only); indie tools barely do better (Open WebUI Automations = RRULE cron; Witsy has webhooks). A local agent can watch the filesystem (fs.watch), expose a localhost webhook endpoint, and poll RSS/IMAP — extending our existing scheduler from "when it's 8am" to "when something happens".
